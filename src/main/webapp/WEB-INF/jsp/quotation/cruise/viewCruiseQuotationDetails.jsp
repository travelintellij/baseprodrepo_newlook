...
<br>
<h2 align="center">View Cruise Quotation Details </h2>
<body>
...
<form:form modelAttribute="LEAD_OBJ" action="create_create_lead_quotation">
	...
	<h3 align="center"><font color="blue"><b>View Cruise Details</b></font></h3>
...
<div class="float-container">
    ...
    <div class="blue">
		...
		<caption><font size="4"> <b>Add Cruise Stay to quotation.</b></font></caption>
			<tr>
				<td style="text-align:center;">
				<ul>
					<c:if test="${QTN_OBJ.converted ne true }">
			  			<li><a href="#"> <input type="button" style="background-color:blue;" value="Search Cruise" /></a></li>
			  			<li><a href="form_view_add_cruise_quotation?leadId=${LEAD_OBJ.leadId }&quotationId=${QTN_OBJ.quotationId}"><input type="button" style="background-color:green;" value="Add Manually" /></a></li>
			  		</c:if>
			  		<c:if test="${QTN_OBJ.converted eq true }">
			  			<li><a href="#"> <input type="button" style="background-color:lightgray;" value="Search Cruise" /></a></li>
		...
		<c:forEach items="${QTN_OBJ.cruiseVoList}" var="cruiseQtnObj">
			<c:choose>
				...
				<c:otherwise>
					...
					<tr><th style="background-color:blue;" colspan="5"><font size="4"><b>Id-  ${cruiseQtnObj.manualCruiseQuotationId}</b></font></th></tr>
					<tr>
					...
					<tr>
						<td><fmt:formatDate value="${cruiseQtnObj.sailingStartDate}" pattern="dd-MM-yyyy" /> </td>
						<td><fmt:formatDate value="${cruiseQtnObj.sailingEndDate}" pattern="dd-MM-yyyy" /></td>
						<td>${cruiseQtnObj.adults}</td>
						<td>${cruiseQtnObj.children}</td>
					...
					<tr>	
						...
						<td colspan="2">${cruiseQtnObj.cruiseStayCost}	</td>
						<td>${cruiseQtnObj.cruiseStayMarkup}</td>
						<td>${cruiseQtnObj.displayOrder }		</td>
					</tr>
					<tr>	
						<th colspan="5"><font size="4"><b>Remarks</b></font></th>
					</tr>
					<tr>	
						<td colspan="5">${cruiseQtnObj.remarks}</td>
					</tr>
					<tr>
						<th colspan="5">
							<c:if test="${QTN_OBJ.converted ne true }">
								<a href="form_view_edit_manual_cruise_quotation?manualCruiseQuotationId=${cruiseQtnObj.manualCruiseQuotationId}&quotationId=${QTN_OBJ.quotationId}&leadId=${QTN_OBJ.leadEntity.leadId}"><input type="button" style="background-color:blue;" value="Edit Stay" /></a>
								<a href="form_view_delete_manual_cruise_quotation?manualCruiseQuotationId=${cruiseQtnObj.manualCruiseQuotationId}&quotationId=${QTN_OBJ.quotationId}&leadId=${QTN_OBJ.leadEntity.leadId}"><input type="button" style="background-color:red;" value="Delete Stay" /></a>
							</c:if>
							<c:if test="${QTN_OBJ.converted eq true }">
